...
def handler(job):
    ...
    # RunPod caps one request at 10 MiB, so the bridge sends the pictures in batches first:
    #   {op: "put", name, images: [{image, caption, index}]}  -> stored on the volume
    # then starts the run with {op: "train", name, useVolume: true, ...}.
    stash = f"{VOL}/train/{name}"
    if inp.get("op") == "put":
        os.makedirs(stash, exist_ok=True)
        if inp.get("reset"):
            shutil.rmtree(stash, ignore_errors=True); os.makedirs(stash, exist_ok=True)
        n = 0
        for im in images:
            data = base64.b64decode(str(im.get("image", "")).split(",")[-1])
            if len(data) < 50:
                continue
            i = int(im.get("index", n))
            ext = "png" if data[:4] == b"\x89PNG" else "jpg"
            with open(f"{stash}/{i:03d}.{ext}", "wb") as f:
                f.write(data)
            with open(f"{stash}/{i:03d}.txt", "w") as f:
                f.write(str(im.get("caption") or f"{trigger}, photo of a woman"))
            n += 1
        return {"stored": n, "total": len([x for x in os.listdir(stash) if not x.endswith(".txt")])}
    if inp.get("useVolume") and os.path.isdir(stash):
        stored = sorted(x for x in os.listdir(stash) if not x.endswith(".txt"))
        for x in stored:
            cap = ""
            try:
                cap = open(f"{stash}/{os.path.splitext(x)[0]}.txt").read()
            except Exception:
                pass
            images.append({"file": f"{stash}/{x}", "caption": cap})
    if len(images) < 4:
